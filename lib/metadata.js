    ...
    for (var propKey in model.entityTypes[typeKey]) {
      var property = model.entityTypes[typeKey][propKey]
        if (propKey=='ODataRelatedTo'){
            console.log('Processing ODataRelatedTo' && model.entityTypes[typeKey][propKey]!=null);
            for (var ODataRelatedTo in model.entityTypes[typeKey][propKey]) {
                console.log('Processing ODataRelatedFrom [' + ODataRelatedTo + ']=' + model.entityTypes[typeKey][propKey][ODataRelatedTo]);
            }
        }else if (propKey=='ODataRelatedFrom' && model.entityTypes[typeKey][propKey]!=null){
            for (var ODataRelatedFrom in model.entityTypes[typeKey][propKey]) {
                console.log('Processing ODataRelatedFrom [' + ODataRelatedFrom + ']=' + model.entityTypes[typeKey][propKey][ODataRelatedFrom]);
            }
        }
