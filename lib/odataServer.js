...
// var BaseRoute = '/GRAX.svc';
var BaseRoute = '';
var GRAXUrl = '';

function ODataServer (serviceUrl) {
  this.serviceUrl = serviceUrl;
  GRAXUrl = this.serviceUrl;
  this.cfg = {
...
ODataServer.prototype.handle = function (req, res) {
  ...
  console.log('[GRAX.io] this.cfg.serviceUrl: ' + this.cfg.serviceUrl);

  var prefix = url.parse(this.cfg.serviceUrl).pathname;

  console.log('[GRAX.io] prefix: ' + prefix);

  if (!this.router || (prefix !== this.router.prefix)) {
    console.log('[GRAX.io] Initializing Routes');
    this.router = new Router(prefix);
    this._initializeRoutes();
  }else
    console.log('[GRAX.io] Skipping Initializing Routes');
...
ODataServer.prototype._initializeRoutes = function () {
  var self = this;
  this.router.get(BaseRoute + '/\$metadata', function (req, res) {
    console.log('[GRAX.io] Servering Meta Data');
    ...
    result = '{"@odata.context":"https://neo4j-odata-prototype.herokuapp.com/GRAX.svc/$metadata","value":[{"name":"Account","kind":"EntitySet","url":"Account"}}';
    return res.end(result)
  })
  this.router.get(BaseRoute + '/:collection/\$count/', function (req, res) {
    req.params.$count = true
    query(self.cfg, req, res)
  })
  this.router.get(BaseRoute + '/:collection\\(:id\\)', function (req, res) {
    query(self.cfg, req, res)
  })
  this.router.get(BaseRoute + '/:collection', function (req, res) {
    query(self.cfg, req, res)
  })
  this.router.get(BaseRoute, function (req, res) {
        // var result = collections(self.cfg)
        // res.writeHead(200, self.cfg.addCorsToHeaders({'Content-Type': 'application/json'}))
        console.log('[GRAX.io] Servering Base Route');
        res.writeHead(200, getheaders(''));
        return res.end(getdefinition())
  })
  this.router.get(BaseRoute + '/', function (req, res) {
    // var result = collections(self.cfg)
      // res.writeHead(200, self.cfg.addCorsToHeaders({'Content-Type': 'application/json;odata.metadata=minimal;odata.streaming=true;IEEE754Compatible=false;charset=utf-8'}))
     // res.writeHead(200, self.cfg.addCorsToHeaders({'Content-Type': 'application/xml', 'DataServiceVersion': '4.0', 'OData-Version': '4.0'}))
      console.log('[GRAX.io] Servering Base Route');
      res.writeHead(200, getheaders(''));
      return res.end(getdefinition())
  })
  this.router.post(BaseRoute + '/:collection', function (req, res) {
    insert(self.cfg, req, res)
  })
  this.router.patch(BaseRoute + '/:collection\\(:id\\)', function (req, res) {
    update(self.cfg, req, res)
  })
  this.router.delete(BaseRoute + '/:collection\\(:id\\)', function (req, res) {
    remove(self.cfg, req, res)
  })

  if (this.cfg.cors) {
    this.router.options(BaseRoute + '/(.*)', function (req, res) {
        console.log('[GRAX.io] Routing /(.*)');
...
function getheaders(contenttype){
  if (contenttype==null || contenttype=='')
      contenttype = 'application/xml;odata.metadata=minimal;odata.streaming=true;IEEE754Compatible=false;charset=utf-8';
...
function getdefinition(){
    var result = '<?xml version="1.0" encoding="UTF-8"?>' +
        '<service xmlns="http://www.w3.org/2007/app" xmlns:atom="http://www.w3.org/2005/Atom" xmlns:m="http://docs.oasis-open.org/odata/ns/metadata" xml:base="' + GRAXUrl + '/" m:context="' + GRAXUrl + '/$metadata">' +
        '<workspace>' +
        '<atom:title type="text">Default</atom:title>' +
        '<collection href="Account">' +
        '<atom:title type="text">Account</atom:title>' +
        '</collection>' +
        '<collection href="Contact">' +
        '<atom:title type="text">Contact</atom:title>' +
        '</collection>' +
        '<collection href="Opportunity">' +
        '<atom:title type="text">Opportunity</atom:title>' +
        '</collection>' +
        '<collection href="Case">' +
        '<atom:title type="text">Case</atom:title>' +
        '</collection>' +
        '<collection href="Asset">' +
        '<atom:title type="text">Asset</atom:title>' +
        '</collection>' +
        '<collection href="Vendor__c">' +
        '<atom:title type="text">Vendor__c</atom:title>' +
        '</collection>' +
        '<collection href="Technology__c">' +
        '<atom:title type="text">Technology__c</atom:title>' +
        '</collection>' +
        '<collection href="Resource__c">' +
        '<atom:title type="text">Resource__c</atom:title>' +
        '</collection>' +
        '<collection href="Application__c">' +
        '<atom:title type="text">Application__c</atom:title>' +
        '</collection>' +
        '<collection href="Program__c">' +
        '<atom:title type="text">Program__c</atom:title>' +
        '</collection>' +
        '<collection href="Project__c">' +
        '<atom:title type="text">Project__c</atom:title>' +
        '</collection>' +
        '<collection href="Project_Application__c">' +
        '<atom:title type="text">Project_Application__c</atom:title>' +
        '</collection>' +
        '</workspace>' +
        '</service>';
    return result;
